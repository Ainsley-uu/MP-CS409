...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        row, col = self.zero_loc[0], self.zero_loc[1]
        for i, j in [(row+1, col), (row, col-1), (row-1, col), (row, col+1)]:
            if 0 <= i < 3 and 0 <= j < 3:
                self.state[i][j], self.state[row][col] = self.state[row][col], self.state[i][j]
                nbr = EightPuzzleState(copy.deepcopy(self.state), self.goal, self.dist_from_start+1, self.use_heuristic, [i, j])
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                if self.goal[i][j] != 0:
                    goal_map[self.goal[i][j]] = (i, j)
        for i in range(3):
            for j in range(3):
                if self.state[i][j] != 0:
                    total += manhattan((i, j), (goal_map[self.state[i][j]]))
