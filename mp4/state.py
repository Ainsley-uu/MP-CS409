...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbor in neighboring_locs:
            ...
            nbr_states.append( SingleGoalGridState(neighbor, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors))
        return nbr_states
    ...
    def __lt__(self, other):
        f_self = self.compute_heuristic() + self.dist_from_start
        f_other = other.compute_heuristic() + other.dist_from_start
        if f_self < f_other:
            return True
        elif f_self == f_other:
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbor in neighboring_locs:
            if neighbor in self.goal:
                idx = self.goal.index(neighbor)
                n_goal = self.goal[:idx] + self.goal[idx+1:]
            else:
                n_goal = self.goal
            nbr_states.append( GridState(neighbor, n_goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors, self.mst_cache))
        return nbr_states
    ...
    def compute_heuristic(self):
        min_mh = float('inf')
        for g in self.goal:
            mh = abs(self.state[0] - g[0]) + abs(self.state[1] - g[1])
            if mh < min_mh:
                min_mh = mh
        ...
        if self.goal in self.mst_cache:
            ...
            mst = compute_mst_cost(self.goal, lambda x, y:abs(x[0]-y[0]) + abs(x[1]-y[1]))
            self.mst_cache[self.goal] = mst
        if not self.goal:
            min_mh = 0
        # manhattan(state, closest_goal) + MST(goals)
        return min_mh + mst
