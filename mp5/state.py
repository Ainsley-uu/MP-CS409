...
class MazeState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for loc in neighboring_locs:
            # print(loc)
            nbr = (loc[0], loc[1])
            cur = (self.state[0], self.state[1])
            if nbr == cur:
                nbr = MazeState(loc, self.goal, self.dist_from_start + 10., self.maze, self.use_heuristic)
            else:
                nbr = MazeState(loc, self.goal, self.dist_from_start + euclidean_distance(nbr, cur), self.maze, self.use_heuristic)
    ...
    def compute_heuristic(self):
        ...
        if self.is_goal():
            return 0
        return min([euclidean_distance(cur, goal) for goal in self.goal])
