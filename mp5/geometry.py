# geometry.py
# ---------------
# Licensing Information:  You are free to use or extend this shadowects for
...
def does_alien_touch_wall(alien: Alien, walls: List[Tuple[int]]):
    ...
    if alien.is_circle():
        for startx, starty, endx, endy in walls:
            w_pos = [[startx, starty], [endx, endy]]
            a_pos = alien.get_centroid()
            if point_segment_distance(a_pos, w_pos) <= alien.get_width():
                return True
    else:
        for startx, starty, endx, endy in walls:
            w_pos = [[startx, starty], [endx, endy]]
            ...
            a_pos = [head, tail]
            if segment_distance(a_pos, w_pos) <= alien.get_width():
                return True
    return False
...
def is_alien_within_window(alien: Alien, window: Tuple[int]):
    ...
    if alien.is_circle():
        ...
        r = alien.get_width()
        if x - r <= 0 or x + r >= window[0] or y - r <= 0 or y + r >= window[1]:
            return False
        return True
    else:
        head, tail = alien.get_head_and_tail()
        ...
        r = alien.get_width()
        if head_x + r >= window[0] or tail_x + r >= window[0] or head_x - r <= 0 or tail_x - r <= 0:
            return False
        if head_y + r >= window[1] or tail_y + r >= window[1] or head_y - r <= 0 or tail_y - r <= 0:
            return False
        return True
...
def is_point_in_polygon(point, polygon):
    ...
    if Ax == Bx == Cx == Dx:
        ...
        if point_segment_distance(point, ((Ax, min_y), (Ax, max_y))) == 0:
            return True
        return False
        ...
        if point_segment_distance(point, ((min_x, Ay), (max_x, Ay))) == 0:
            return True
    else:
        x, y = point[0], point[1]
...
def does_alien_path_touch_wall(alien: Alien, walls: List[Tuple[int]], waypoint: Tuple[int, int]):
    """Determine whether the alien's straight-line path from its current position to the waypoint touches a wall
        ...
        Return:
            True if touched, False if not
    """
    next_alien = deepcopy(alien)
    next_alien.set_alien_pos(waypoint)
    min_dist = alien.get_width()
    if alien.is_circle():
        if does_alien_touch_wall(alien, walls) or does_alien_touch_wall(next_alien, walls):
            return True
        for startx, starty, endx, endy in walls:
            pos1 = [(startx, starty), (endx, endy)]
            pos2 = [alien.get_centroid(), waypoint]
            if segment_distance(pos1, pos2) <= min_dist:
                return True
    else:
        for startx, starty, endx, endy in walls:
            wall = [(startx, starty), (endx, endy)]
            head1, tail1 = alien.get_head_and_tail()
            head2, tail2 = next_alien.get_head_and_tail()
            polygon = (head1, tail1, tail2, head2)
            if is_point_in_polygon(wall[0], polygon):
                return True
            if  is_point_in_polygon(wall[1], polygon):
                return True
            for t in [head1, tail1], [head2, tail2], [head1, head2], [tail1, tail2]:
                if (segment_distance(wall, t) <= min_dist):
                    return True
    return False
...
def point_segment_distance(p, s):
    ...
    shadow = np.dot(p, segment) / LA.norm(segment)

    if shadow <= 0:  
        return LA.norm(p)
    if shadow >= LA.norm(segment):  
        return LA.norm(p - segment)
    if np.isclose(LA.norm(p), shadow):  
        return 0
    return (LA.norm(p) ** 2 - shadow ** 2) ** 0.5
...
def do_segments_intersect(s1, s2):
    ...
    if (any(point_segment_distance(p, s2) == 0 for p in s1)
        ...
        return True
    if np.cross(s2[0]-s1[0], s1[1]-s1[0]) * np.cross(s2[1]-s1[0], s1[1]-s1[0]) >= 0:
        return False
    
    if np.cross(s1[0]-s2[0], s2[1]-s2[0]) * np.cross(s1[1]-s2[0], s2[1]-s2[0]) >= 0:
        return False
    return True
